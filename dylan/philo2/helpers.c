...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/19 04:06:40 by dlesieur          #+#    #+#             */
/*   Updated: 2025/10/19 04:47:01 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_philo_death(t_data *c, int i)
{
	...
	if (current_time - last_meal_time > c->args[TT_DIE])
	{
		...
		{
			c->finished = 1;
			c->sim_state = SIM_STATE_DEATH;
			pthread_mutex_unlock(&c->finished_m);
			set_simulation_state(c, 0);
			log_debug(&c->phi[i], LOG_DEATH, "Death detected by monitor");
